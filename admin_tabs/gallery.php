                ...
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <div style="display: flex; gap: 4px;">
                        <button type="button" class="btn-icon" 
                            onclick="viewGalleryImage('<?= htmlspecialchars($img['image_url']) ?>', '<?= htmlspecialchars($img['title'] ?: 'Image') ?>', '<?= htmlspecialchars($img['description']) ?>')"
                            style="padding: 6px 10px; font-size: 12px; border-radius: 8px; border: none; background: #e0e7ff; color: #4361ee; cursor: pointer;"
                            title="View Image">
                            <i class="fa-solid fa-eye"></i>
                        </button>
                        <button type="button" class="btn-icon" 
                            onclick="openEditGalleryModal(<?= htmlspecialchars(json_encode($img)) ?>)"
                            style="padding: 6px 10px; font-size: 12px; border-radius: 8px; border: none; background: #fef3c7; color: #d97706; cursor: pointer;"
                            title="Edit Details">
...
<script>
    ...
    function viewGalleryImage(src, title, desc) {
        document.getElementById('view_img_src').src = src;
        document.getElementById('view_img_title').innerText = title;
        document.getElementById('view_img_desc').innerText = desc || 'No description available.';
        document.getElementById('viewGalleryModal').style.display = 'flex';
    }
    ...
    function openEditGalleryModal(data) {
        ...
        modal.querySelector('select[name="category"]').value = data.category;
        modal.querySelector('textarea[name="description"]').value = data.description;
        modal.querySelector('input[name="existing_image"]').value = data.image_url;
        document.getElementById('edit_gallery_preview').src = data.image_url;
        document.getElementById('edit_gallery_preview').style.display = 'block';
        document.getElementById('edit_gallery_placeholder').style.display = 'none';
...
<!-- Edit Gallery Modal -->
<div class="modal-overlay" id="editGalleryModal" style="display: none;">
    <div class="modal-content" style="max-width: 520px;">
        <div class="card-header" style="border-bottom:none; margin-bottom:5px; padding-bottom:0;">
            <span class="card-title" style="font-size: 20px;"><i class="fa-solid fa-pen-to-square"
                    style="margin-right:8px; color:#d97706;"></i>Edit Gallery Image</span>
            <button type="button" onclick="document.getElementById('editGalleryModal').style.display='none';" class="btn"
                style="background:none; border:none; color:#888; font-size:22px; cursor:pointer; padding:0;"><i
                    class="fa-solid fa-xmark"></i></button>
        </div>
        <form method="POST" enctype="multipart/form-data">
            ...
            <div style="margin-top: 15px; text-align: center;">
                <div style="width: 100%; height: 220px; background: #f8fafc; border: 2px dashed #cbd5e1; border-radius: 14px; display: flex; align-items: center; justify-content: center; overflow: hidden; position: relative; margin-bottom: 20px; cursor: pointer; transition: border-color 0.2s;"
                    ...
                    <input type="file" name="gallery_photo" accept="image/*"
                        style="position: absolute; width: 100%; height: 100%; opacity: 0; cursor: pointer; z-index: 2;"
                        onchange="previewMenuImage(this, 'edit_gallery_preview', 'edit_gallery_placeholder')">
                    <div id="edit_gallery_placeholder" style="display:none; text-align: center; color: #94a3b8; pointer-events: none;">
                        <i class="fa-solid fa-cloud-arrow-up"
                            style="font-size: 36px; margin-bottom: 12px; color: #4361ee;"></i><br>
                        <strong style="color: #475569;">Click or drag to change</strong><br>
                        <small>JPG, PNG, GIF, WEBP</small>
                    </div>
                    <img id="edit_gallery_preview" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px;">
                <button type="button" class="btn"
                    onclick="document.getElementById('editGalleryModal').style.display='none';"
                    style="background: #f1f5f9; color: #475569; border-radius: 8px; padding: 12px 20px; font-weight: 600;">Cancel</button>
                <button type="submit" class="btn"
                    style="background: linear-gradient(135deg, #d97706, #b45309); color: #fff; border-radius: 8px; padding: 12px 25px; font-weight: 600; border:none; cursor:pointer; box-shadow: 0 4px 12px rgba(217,119,6,0.25);">
...
<!-- View Gallery Modal (Lightbox style) -->
<div class="modal-overlay" id="viewGalleryModal" style="display: none; background: rgba(0,0,0,0.9);">
    <div style="position: absolute; top: 20px; right: 20px; z-index: 100;">
        <button type="button" onclick="document.getElementById('viewGalleryModal').style.display='none';" 
            style="background: rgba(255,255,255,0.1); border: none; color: #fff; font-size: 30px; cursor: pointer; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; backdrop-filter: blur(5px);">
            <i class="fa-solid fa-xmark"></i>
        </button>
    </div>
    <div style="width: 90%; max-width: 1000px; display: flex; flex-direction: column; align-items: center; gap: 20px;">
        <img id="view_img_src" style="max-width: 100%; max-height: 70vh; border-radius: 12px; box-shadow: 0 10px 50px rgba(0,0,0,0.5);">
        <div style="text-align: center; color: #fff; background: rgba(0,0,0,0.5); padding: 20px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(10px); width: 100%;">
            <h3 id="view_img_title" style="margin: 0 0 10px; font-size: 24px; color: #fff;"></h3>
            <p id="view_img_desc" style="margin: 0; color: #cbd5e1; font-size: 15px; line-height: 1.6;"></p>
        </div>
    </div>
</div>