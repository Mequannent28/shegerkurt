<script>
    ...
    function showIDCard(emp) {
        ...
        document.getElementById('id_card_no').innerText = emp.id_number || ('SK-' + String(emp.id).padStart(3, '0'));
        ...
        else { photoImg.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(emp.name)}&background=f59e0b&color=fff&size=200`; }

        // Update QR code with employee data
        const qrData = `ShegerKurtID|${emp.id_number}|${emp.name}`;
    ...
    function generateIDFromForm() {
        ...
        const emp = {
            id: document.getElementById('edit_emp_id').value || '000',
            id_number: 'SK-' + String(document.getElementById('edit_emp_id').value || '000').padStart(3, '0'),
            title: title,
            name: name,
            ...
            photo: document.getElementById('edit_emp_photo_preview').src.includes('http') || document.getElementById('edit_emp_photo_preview').src.includes('data:') ? document.getElementById('edit_emp_photo_preview').src : null
        };
        showIDCard(emp);
    }
