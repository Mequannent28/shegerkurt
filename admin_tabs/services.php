<div
    ...
    <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
        <!-- Bulk Actions -->
        <div id="services_bulk_actions" style="display: none; align-items: center; gap: 10px;">
            <span id="services_selected_count" style="font-size: 13px; font-weight: 700; color: #2563eb;">0
                selected</span>
            <form method="POST" id="services_bulk_form" style="display: flex; gap: 5px;">
                <input type="hidden" name="services_bulk_ids" id="services_bulk_ids_input">
                <button type="submit" name="bulk_delete_services" class="btn"
        ...
        <label
            ...
            <input type="checkbox" id="select_all_services" onchange="toggleSelectAllServices(this)"
...
<div class="grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">
    ...
    foreach ($services as $s):
        ...
        ?>
        <div class="card"
            style="padding: 0; overflow: hidden; border-radius: 15px; border: 1px solid #e2e8f0; background: #fff; position: relative;">
            <!-- Checkbox -->
            <div style="position: absolute; top: 10px; left: 10px; z-index: 5;">
                <input type="checkbox" class="services-checkbox" value="<?= $s['id'] ?>" onchange="updateServicesBulkUI()"
            ...
            <div style="padding: 20px;">
                ...
                <div
                    ...
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;">
                        <button type="button" class="btn-icon view-service-btn"
                            style="color: #0ea5e9; background: #e0f2fe; border-radius: 8px; padding: 8px 12px; border: none; cursor: pointer;"
...
<script>
    function toggleSelectAllServices(source) {
        document.querySelectorAll('.services-checkbox').forEach(cb => cb.checked = source.checked);
        updateServicesBulkUI();
    }
    function updateServicesBulkUI() {
        const checked = document.querySelectorAll('.services-checkbox:checked');
        const all = document.querySelectorAll('.services-checkbox');
        const bulk = document.getElementById('services_bulk_actions');
        const count = document.getElementById('services_selected_count');
        const ids = document.getElementById('services_bulk_ids_input');
        const selAll = document.getElementById('select_all_services');
        if (checked.length > 0) {
...
<script>
    ...
    document.querySelectorAll('.view-service-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
            var s = JSON.parse(this.getAttribute('data-service'));
            document.getElementById('view_service_title').innerText = s.title;
            document.getElementById('view_service_category').innerText = s.category;
            document.getElementById('view_service_status').innerText = s.status;
            document.getElementById('view_service_description').innerText = s.description;
            
            const img = document.getElementById('view_service_img');
            if (s.image_url) {
                img.src = s.image_url;
            ...
            const videoArea = document.getElementById('view_service_video_area');
            if (s.video_url) {
                videoArea.innerHTML = `<a href="${s.video_url}" target="_blank" style="color:#2563eb; text-decoration:none;"><i class="fa-solid fa-play-circle"></i> Watch Promo Video</a>`;
...
<div id="viewServiceModal" 
    style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.6); z-index: 1200; align-items: center; justify-content: center; backdrop-filter: blur(5px);">
    <div style="background: #fff; width: 90%; max-width: 600px; border-radius: 20px; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); animation: serviceSlideUp 0.3s ease;">
        <div style="height: 250px; background: #f8fafc; position: relative; overflow: hidden;">
            <img id="view_service_img" style="width: 100%; height: 100%; object-fit: cover;">
            <div style="position: absolute; top: 20px; right: 20px;">
                <button type="button" onclick="document.getElementById('viewServiceModal').style.display='none';" 
                    style="background: rgba(255,255,255,0.9); border: none; color: #1e293b; width: 40px; height: 40px; border-radius: 50%; font-size: 18px; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(0deg, rgba(0,0,0,0.7), transparent); padding: 30px 25px;">
                <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                    <span id="view_service_category" style="background: rgba(255,255,255,0.2); backdrop-filter: blur(5px); color: #fff; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; text-transform: uppercase;"></span>
                    <span id="view_service_status" style="background: #10b981; color: #fff; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; text-transform: uppercase;"></span>
                </div>
                <h2 id="view_service_title" style="margin: 0; color: #fff; font-size: 26px; font-weight: 800;"></h2>
            </div>
        </div>
        <div style="padding: 30px;">
            <p id="view_service_description" style="color: #475569; line-height: 1.8; font-size: 15px; margin: 0 0 25px;"></p>
            <div id="view_service_video_area" style="padding: 15px; background: #eff6ff; border-radius: 12px; margin-bottom: 25px; font-weight: 600; display: none;"></div>
            <button onclick="document.getElementById('viewServiceModal').style.display='none';" class="btn" 
                style="width: 100%; padding: 15px; border-radius: 12px; background: #1e293b; color: #fff; font-weight: 700; border: none; cursor: pointer;">
                Close Service Overview
            </button>
        </div>
    </div>
</div>